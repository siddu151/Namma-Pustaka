...
import com.example.nammapustaka.adapters.OverdueRowsAdapter
import com.example.nammapustaka.databinding.FragmentOverdueBinding
...
class OverdueFragment : Fragment() {
    ...
    private val vm: OverdueViewModel by viewModels {
        AppViewModelFactory.from(requireActivity().application as NammaPustakaApp)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentOverdueBinding.inflate(inflater, container, false)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        binding.toolbar.setNavigationOnClickListener { findNavController().popBackStack() }
        val adapter = OverdueRowsAdapter()
